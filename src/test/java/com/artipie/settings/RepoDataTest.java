...
class RepoDataTest {
    ...
    void removesData() {
        ...
        new RepoData(new ManageRepoSettings(this.stngs))
            .remove(RepositoryName.create(RepoDataTest.REPO)).toCompletableFuture().join();
    ...
    void movesData() {
        ...
        this.data.save(new Key.From(RepoDataTest.REPO, "second.txt"), new byte[]{});
        new RepoData(new ManageRepoSettings(this.stngs))
            .move(RepositoryName.create(RepoDataTest.REPO), RepositoryName.create("new-repo"))
    ...
    void movesDataWithAliasAndOrgLayout(final String key) {
        ...
        new RepoData(new ManageRepoSettings(this.stngs))
            .move(
                new RepositoryName(RepoDataTest.REPO, uid, "org"),
                new RepositoryName(nrepo, uid, "org")
            )
            .toCompletableFuture().join();
        MatcherAssert.assertThat(
            "Repository data are moved",
    ...
    void removesDataWithAliasAndOrgLayout(final String key) {
        ...
        this.data.save(new Key.From(uid, RepoDataTest.REPO, "second.txt"), new byte[]{});
        new RepoData(new ManageRepoSettings(this.stngs))
            .remove(new RepositoryName(RepoDataTest.REPO, uid, "org")).toCompletableFuture().join();
        MatcherAssert.assertThat(
            "Repository is empty",
            this.data.list(Key.ROOT).isEmpty()
        );
